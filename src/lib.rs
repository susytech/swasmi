...
//! extern crate wabt;
//!
//! use swasmi::{ModuleInstance, ImportsBuilder, NopExternals, RuntimeValue};
//!
//! fn main() {
...
//!     let module = swasmi::Module::from_buffer(&wasm_binary)
...
pub struct Module {
	...
	module: swasm::elements::Module,
}

impl Module {
	/// Create `Module` from `swasm::elements::Module`.
	///
	/// This function will load, validate and prepare a `swasm`'s `Module`.
	///
	/// # Errors
	///
	...
	///         .build();
	///
	///     let module = swasmi::Module::from_swasm_module(susy_module)
	///         .expect("swasm builder generated invalid module!");
	///
	///     // Instantiate `module`, etc...
	/// }
	/// ```
	pub fn from_swasm_module(module: swasm::elements::Module) -> Result<Module, Error> {
	...
	///
	/// // Load wasm binary and prepare it for instantiation.
	/// let module = swasmi::Module::from_buffer(&wasm_binary).expect("Parsing failed");
	...
	pub fn from_buffer<B: AsRef<[u8]>>(buffer: B) -> Result<Module, Error> {
		let module = swasm::elements::deserialize_buffer(buffer.as_ref())
			.map_err(|e: swasm::elements::Error| Error::Validation(e.to_string()))?;
		Module::from_swasm_module(module)
	}

	pub(crate) fn module(&self) -> &swasm::elements::Module {
		&self.module
	}
