...
use swasm::elements::{External, InitExpr, Internal, Instruction, ResizableLimits, Type};
...
impl ModuleInstance {
	...
	///
	/// ```rust
	/// use swasmi::{ModuleInstance, ImportsBuilder, NopExternals};
	/// # fn func() -> Result<(), ::swasmi::Error> {
	/// # let module = swasmi::Module::from_buffer(&[0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00]).unwrap();
	///
	...
	/// # let module = swasmi::Module::from_buffer(&wasm_binary).expect("failed to load wasm");
